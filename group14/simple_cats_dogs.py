...
df_SGD = pd.DataFrame({'a': used_alpha, 'loss': used_loss, 'Training': train_acc_SGD, 'Validation': valid_acc_SGD})
df_SGD = df_SGD.set_index('a')

p2 = sns.lineplot(data=df_SGD.loc[df_SGD['loss'] == 'log'], linewidth=2.5, dashes=False)
p2.set(xlabel = 'alpha', ylabel = 'Accuracy', title='SGD log loss')
p2.axhline(test_acc_SGD, color='g', linestyle='--')
plt.annotate('Testing accuracy', xy=(20, 0.605), color='g')
plt.ylim(y_min, y_max)
plt.savefig('SGD_log.png')
plt.close()

p3 = sns.lineplot(data=df_SGD.loc[df_SGD['loss'] != 'log'], linewidth=2.5, dashes=False)
p3.set(xlabel = 'alpha', ylabel = 'Accuracy', title='SGD modified_huber loss')
p3.axhline(test_acc_SGD, color='g', linestyle='--')
plt.annotate('Testing accuracy', xy=(10, 0.605), color='g')
plt.ylim(y_min, y_max)
